...
export default function CustomerLoginPage() {
    ...
    const handlePhoneSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast({
                title: 'OTP Sent',
                description: 'For this demo, please enter "1234" to log in.',
            });
            setStep('otp');
        } catch (error) {
            console.error(error);
    ...
    const handleOtpSubmit = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            const { success } = await verifyOtp({ phone: phoneNumber, code: otp });
            if (success) {
            ...
            <div className="mx-auto w-full max-w-sm gap-6">
                ...
                {step === 'otp' ? (
                    ...
                    <form onSubmit={handlePhoneSubmit} className="space-y-4 pt-4">
                        ...
                        <Button type="submit" className="w-full h-11" disabled={isLoading || !phoneNumber}>
                            {isLoading ? (
                                <>
                                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                    Sending OTP...
