...
import { FileDown, FileUp, Trash2, Plus, Search } from 'lucide-react';
...
export default function DataPage() {
  ...
  const { driverGridData, addDriver, removeDrivers } = useDriverStore();
      ...
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <div className="flex gap-2">
          {isProfileLoading ? (
            ...
            <>
              <Button onClick={() => setIsAddDriverOpen(true)} disabled={!isAdmin}>
          ...
          <Table className="min-w-full divide-y divide-gray-200">
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Allowed Vehicles</TableHead>
                ...
                <TableHead>Last FC</TableHead>
                <TableHead>FC Expiry</TableHead>
                <TableHead>Permit</TableHead>
                <TableHead>PUCC Expiry</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredData &&
                filteredData.map((driver, index) => (
                  <TableRow key={index} data-state={selectedRows.includes(driver.dlNo) && "selected"} className="hover:bg-muted/50">
                    <TableCell>
                        <Checkbox
                    ...
                    <TableCell>{driver.allowedVehicles}</TableCell>
                    <TableCell>{driver.vehicleRegNum}</TableCell>
                    <TableCell>{driver.vehicleType}</TableCell>
                    <TableCell>{driver.make}</TableCell>
                    <TableCell>{driver.model}</TableCell>
                    <TableCell>{driver.capacity}</TableCell>
                    <TableCell>{driver.insuranceExpiry}</TableCell>
                    <TableCell>{driver.lastFC}</TableCell>
                    <TableCell>{driver.fcExpiry}</TableCell>
                    <TableCell>{driver.permit}</TableCell>
                    <TableCell>{driver.puccExpiry}</TableCell>
                  </TableRow>
                ))}
                {filteredData.length === 0 && (
                    <TableRow>
                        <TableCell colSpan={16} className="text-center h-24">No results found.</TableCell>
                    </TableRow>
                )}
            </TableBody>
          </Table>
