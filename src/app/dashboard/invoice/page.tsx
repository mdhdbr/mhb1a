...
import { Mail, PlusCircle, Trash2, Printer, Download, CheckCircle, Archive, Search, ArrowLeft, Filter, MessageSquare } from "lucide-react";
...
const GeneratedInvoiceView = ({ invoiceData }: { invoiceData: InvoiceData }) => {
    ...
    const handleSendEmail = () => {
        ...
        const subject = `Invoice ${invoiceData.jobId} from Pro Seed`;
             ...
             <div className="p-4 sm:p-6 md:p-8 border rounded-lg" id="invoice-content">
                <header className="flex justify-between items-start mb-10">
                    ...
                    <div className="text-right">
                        ...
                        <p className="font-semibold text-muted-foreground mt-1">{invoiceData.jobId}</p>
                    </div>
                </header>
    ...
    const handleGenerateClick = () => {
        if (!selectedJobId) {
            toast({
                ...
                description: 'Please select a Job ID to generate an invoice.',
            });
            return;
        }
        ...
        const newInvoiceData: InvoiceData = {
            ...
            billedTo: {
                ...
                email: 'accounts@' + job.client.toLowerCase().replace(/[^a-z0-9]/g, '') + '.com',
            },
            from: {
                name: 'Pro Seed',
            ...
            dueDate: '2026-01-22',
            items: [
                { id: 1, description: `Shipment Transport - ${job.vehicleType}`, kms: parseFloat(job.distance), rate: parseFloat((baseFare / parseFloat(job.distance)).toFixed(2)), amount: baseFare },
                { id: 2, description: `Fuel Surcharge (${fuelSurchargePercent}%)`, amount: fuelSurchargeAmount },
            ],
            notes: 'Please make all payments to the account details provided separately.\nThank you for your business!',
        };
        ...
        toast({
            ...
            description: `Showing invoice for Job ID ${selectedJobId}.`,
        });
    };
    return (
        <div className="space-y-6">
            <Card className="overflow-hidden">
                <CardHeader>
                    <CardTitle className="font-headline text-xl">Generate Invoice from Job</CardTitle>
                    <CardDescription>Select a completed Job ID from the table below to generate a detailed invoice.</CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="flex flex-col sm:flex-row items-center gap-4">
                        <div className="relative flex-grow w-full">
                            <Input
                                id="job-id-display"
                                value={selectedJobId || "No job selected"}
                                readOnly
                                className="h-11 text-base"
                            />
                        </div>
                        <Button onClick={handleGenerateClick} className="w-full sm:w-auto h-11 text-base" disabled={!selectedJobId}>
...
const CompletedJobsTable = ({ completedJobs, selectedJobId, setSelectedJobId, onGenerate }: { completedJobs: JobForInvoice[], selectedJobId: string | null, setSelectedJobId: (id: string | null) => void, onGenerate: () => void }) => {
                ...
                <Table>
                    <TableHeader>
                        <TableRow className="hover:bg-transparent">
                            ...
                            <TableHead className="text-center w-[120px] align-bottom">Generate</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {filteredJobs.map((job) => (
                            <TableRow 
                                key={job.id} 
                                data-state={selectedJobId === job.id ? "selected" : "unselected"}
                                ...
                                <TableCell className="text-center">
                                    <Button 
                                        ...
                                        onClick={(e) => {
                                            ...
                                            // A little delay to show selection then generate
