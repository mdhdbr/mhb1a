...
import { Send, Phone, Video, Search, User, Truck, Building, Loader2, ArrowRight, MessageSquare, Users, Filter } from 'lucide-react';
...
import type { ChatContact, UserProfile } from '@/lib/types';
    ...
    return (
        <ScrollArea className="h-full">
            <div className="flex flex-col gap-1 p-2">
                {contacts.length === 0 && (
                    <div className="text-center text-sm text-muted-foreground p-4">No contacts match your filters.</div>
                )}
                {contacts.map((contact) => {
                    ...
                    return (
                        ...
                        >
                             <ContextMenu menuItems={<ContactContextMenu contact={contact} />}>
...
export default function SmsPage() {
    ...
    // Filtered lists
    const filteredDrivers = useMemo(() => drivers.filter(c => 
        c.name.toLowerCase().includes(searchQuery.toLowerCase()) &&
        (driverStatusFilter === 'all' || c.status === driverStatusFilter)
    ), [drivers, searchQuery, driverStatusFilter]);

    const filteredCustomers = useMemo(() => customerContacts.filter(c => 
        c.name.toLowerCase().includes(searchQuery.toLowerCase()) &&
        (customerRatingFilter === 'all' || c.rating === customerRatingFilter)
    ), [customerContacts, searchQuery, customerRatingFilter]);

    const filteredUsers = useMemo(() => allUsers.filter(c => 
        c.name.toLowerCase().includes(searchQuery.toLowerCase()) &&
        (userRoleFilter === 'all' || c.role === userRoleFilter)
    ), [allUsers, searchQuery, userRoleFilter]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-[350px_1fr] h-[calc(100vh-8rem)] gap-4">
            <Card className="flex flex-col">
                ...
                <Tabs value={activeTab} onValueChange={handleTabChange} className="flex flex-col flex-1 overflow-hidden">
                    ...
                    <TabsContent value="drivers" className="flex-1 mt-0 flex flex-col">
                    ...
                    <TabsContent value="customers" className="flex-1 mt-0 flex flex-col">
                       <div className="p-2 border-b">
                         <Select value={customerRatingFilter} onValueChange={setCustomerRatingFilter}>
                            <SelectTrigger className="w-full h-9">
                                <div className="flex items-center gap-2">
                                <Filter className="h-4 w-4 text-muted-foreground" />
                    ...
                    <TabsContent value="users" className="flex-1 mt-0 flex flex-col">
                        <div className="p-2 border-b">
