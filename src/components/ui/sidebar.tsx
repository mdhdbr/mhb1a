        ...
        <div
          className={cn(
            ...
            variant === "floating" || variant === "inset"
              ...
              : "",
            open
              ? (variant === "floating" || variant === "inset" ? "w-[calc(var(--sidebar-width)_+_theme(spacing.4))]" : "w-[var(--sidebar-width)]")
        ...
        >
          <div
            ...
            className={cn("flex h-full w-full flex-col",
              variant !== 'floating' && variant !== 'inset' && "group-data-[side=left]:border-r group-data-[side=right]:border-l",
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        'flex-1 bg-background transition-[margin-left] duration-200',
...
>(({ className, onKeyDown, ...props }, ref) => {
  const handleKeyDown = (event: React.KeyboardEvent<HTMLUListElement>) => {
    ...
    if (event.key === 'Backspace' || event.key === 'ArrowLeft') {
      ...
      // Find the closest parent submenu content
      const subMenuContent = target.closest('[data-radix-collapsible-content]');
      if (subMenuContent) {
        // Find the trigger button for that submenu
        const subMenuTrigger = subMenuContent.previousElementSibling?.querySelector('button[data-sidebar="menu-button"]');
        if (subMenuTrigger instanceof HTMLElement) {
          event.preventDefault();
          subMenuTrigger.click(); // Close the submenu
          subMenuTrigger.focus(); // Focus on the parent menu item
        }
      }
    }
  };
...
>(({ className, ...props }, ref) => {
  ...
  >
    <ul className={cn(
      "mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border pl-3.5 py-1 data-[state=closed]:hidden",
      !open && "hidden",
      className
    )} />
  </CollapsibleContent>
