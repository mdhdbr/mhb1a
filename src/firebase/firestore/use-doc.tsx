...
export function useDoc<T = any>(
  ...
  const [key, setKey] = useState(0); // Add key state for re-fetching
  const { user, isUserLoading } = useUser(); // Get user and loading state

  const mutate = () => setKey(prev => prev + 1);

  useEffect(() => {
    // **THE GUARANTEED FIX**: Wait for auth to be resolved AND for the doc ref to be ready.
    ...
    return () => unsubscribe();
  }, [memoizedDocRef, key, isUserLoading, user]); // Add user to dependency array
