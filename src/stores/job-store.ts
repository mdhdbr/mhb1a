  ...
  return driverGridData.map((driver, index) => {
    const jobDetails: { [key: string]: VehicleJob } = {
      ...
      'SA-24680': { // Hassan Ibrahim
        ...
        bookingTime: new Date(Date.now() - 3 * 60 * 1000).toISOString(),
        flight: 'SV123',
        pax: '2 PAX',
        distance: 75,
        dwellTime: '5m',
      ...
      // New job for soft_allocated alert
      'SA-97531': { 
        id: 'JOB-SOFT',
        status: 'Soft Allocated',
        service: 'Freight',
        account: 'Modern Industries',
        pickup: 'Jubail Industrial City',
        pickupDate: format(new Date(), 'dd/MM/yyyy HH:mm'),
        bookingTime: new Date(Date.now() - 60 * 60 * 1000).toISOString(),
        flight: null,
        pax: '1 Pallet',
        distance: 85,
        dwellTime: '0m',
